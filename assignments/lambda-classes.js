...
class Instructor extends Person {
  ...
  changeGrade(student) {
    let originalGrade = student.studentgrade;
    let recentChange = Math.floor(Math.random() * 10);
    student.studentgrade += recentChange;
    console.log(
      `${student.name}'s grade has increased from ${originalGrade} by ${recentChange} points to ${student.studentgrade} points,`
    );
    if (student.studentgrade > 70) {
      return `\n\n  ***** YOU GRADUATED ***\n\n`;
    } else {
      return `\n\n   You need to do better on your next test in order to graduate...\n\n`;
    }
  }
} // Instructor
...
console.log(fred.changeGrade(anthony));
...
// console.log(anthony.studentgrade);
